...
def compDiv(spA, spB, segsites, LocLen, numB):
	...
	for i in spA:
		...
		for j in spB:
			...
			if spBcount <= numB/2:
				spB_1 += 1
				spB_rand.append(random.choice([-1,1]))
				if spB_rand[spB_1-1] == 1:
					nAB_1_1 += 1
					divAB_1_1.append(0)
					for k in range(segsites):
						if i[k]!=j[k]:
							div[nPair - 1] += 1
							divAB_1_1[nAB_1_1 - 1] += 1
				else:
					nAB_1_2 += 1
					divAB_1_2.append(0)
					for k in range(segsites):
						if i[k]!=j[k]:
							div[nPair - 1] += 1
							divAB_1_2[nAB_1_2 - 1] += 1
			if spBcount > numB/2:
				spB_2 += 1
				if spB_rand[spB_2-1] == 1:
					nAB_2_1 += 1
					divAB_2_1.append(0)
					for k in range(segsites):
						if i[k]!=j[k]:
							div[nPair - 1] += 1
							divAB_2_1[nAB_2_1 - 1] += 1
				else:
					nAB_2_2 += 1
					divAB_2_2.append(0)
					for k in range(segsites):
						if i[k]!=j[k]:
							div[nPair - 1] += 1
							divAB_2_2[nAB_2_2 - 1] += 1
		divAB_1 = divAB_2_1 + divAB_1_1
		divAB_2 = divAB_1_2 + divAB_2_2
		for i in range(len(divAB_1)):
			for j in range(len(divAB_2)):
				indD3count += 1
				indD3.append(0)
				if divAB_1[i]+divAB_2[j] != 0:
					indD3[indD3count-1] = abs(((divAB_2[j]/LocLen)-(divAB_1[i]/LocLen))/((divAB_2[j]/LocLen)+(divAB_1[i]/LocLen)))
		D3temp[nD3-1] = cr_mean(indD3)
	res = {}
	res['divAB'] = cr_mean(div)
	#res['divAB_1'] = cr_mean(divAB_1)
	#res['divAB_2'] = cr_mean(divAB_2)
	res['D3'] = cr_mean(D3temp)
...
# ms' output file
outfile_jsfs = open("{0}/ABCjsfs.txt".format(modeldir), "w")
header = ''
for i in range(nBins_spA):
...
outfile = open("{0}/ABCstat.txt".format(modeldir), "w")
...
for line in sys.stdin: # read the ms's output from the stdin
		...
		if segsites != 0:
			if "positions" not in line and line!="\n":
				...
				if nSam_cnt == (nSamA[nLoci_cnt - 1] + nSamB[nLoci_cnt - 1]):
					...
					# divAB
					div = compDiv(spA, spB, segsites, L[nLoci_cnt - 1], nSamB[nLoci_cnt - 1])
	...
	if nLoci_cnt != 0 and len(ss) == nLoci:
		...
		res += "{0:.5f}\t{1:.5f}\t".format(D3_avg, D3_std)
		...
		outfile_all = open("{0}/Allstat.txt".format(modeldir), "w")
		res = "locus\tbialsites\t"
		res += "piA\t"
		res += "piB\t"
		res += "thetaA\t"
		...
		res += "FST\t"
		res += "D3a\n"
		outfile_all.write(res)
		for i in range(len(bialsites)):
			res = ""
			res += "{0:.5f}\t".format(i)
			res += "{0:.5f}\t".format(bialsites[i])
			res += "{0:.5f}\t".format(piA[i])
			res += "{0:.5f}\t".format(piB[i])
			res += "{0:.5f}\t".format(thetaA[i])
			res += "{0:.5f}\t".format(thetaB[i])
			res += "{0:.5f}\t".format(DtajA[i])
			res += "{0:.5f}\t".format(DtajB[i])
			res += "{0:.5f}\t".format(divAB[i])
			res += "{0:.5f}\t".format(netdivAB[i])
			res += "{0:.5f}\t".format(FST[i])
			res += "{0:.5f}\t".format(D3[i])
			res += "\n"
			outfile_all.write(res)
		vector_sfs = []
